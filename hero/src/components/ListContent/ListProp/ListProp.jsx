...
class Com extends Component {
    ...
    onChange(e) {
      ...
      onchanged(e) {
        if(e.target.value !== '') {
            api.requestAllPropNameData(e.target.value)
            .then(data => {
            this.setState({
                allList:data.data,
                activeIn: 2
            ...
            api.requestAllPropData(pageCode)
            .then(data => {
                this.setState({
                    allList:data.data,
                    activeIn: 1
    ...
    render () {
        ...
        let named = this.props.match.params.name
            named = '6'
        if(named === '6' && this.state.activeIn === 1) {
            html = <Pagination showQuickJumper  defaultCurrent={1} total={20} onChange={this.onChange.bind(this)} />
        }
        else {
             html = ''
        }
        return (
            <div>
                <div className="navigation">您现在的位置：英雄联盟 > 道具包</div>
                    ...
                    <div className='right'>
                        <div className='right_nav'>
                            <div className='search'>
                               关键字: <input type="text" defaultValue={named === '6' ? '' : named} onChange={this.onchanged.bind(this)}/>
