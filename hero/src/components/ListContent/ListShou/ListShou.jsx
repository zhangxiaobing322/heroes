...
class Com extends Component {
    ...
    onChange(e) {
      ...
      onchanged(e) {
        if(e.target.value !== '') {
            api.requestAllShouWeiNameData(e.target.value)
            .then(data => {
            this.setState({
                allList:data.data,
                activeIn: 2
            ...
            api.requestAllShouWeiData(pageCode)
            .then(data => {
                this.setState({
                    allList:data.data,
                    activeIn: 1
    ...
    render () {
        ...
        let named = this.props.match.params.name
            named = '5'
        if(named === '5' && this.state.activeIn === 1) {
            html = <Pagination showQuickJumper  defaultCurrent={1} total={30} onChange={this.onChange.bind(this)} />
        }
        else {
             html = ''
        }
        return (
            <div>
                <div className="navigation">您现在的位置：英雄联盟 > 守卫眼皮肤</div>
                    ...
                    <div className='right'>
                        <div className='right_nav'>
                            <div className='search'>
                               关键字: <input type="text" defaultValue={named === '5' ? '' : named} onChange={this.onchanged.bind(this)}/>
                                    ...
                                    this.state.allList.slice(0,12).map((item,index)=> {
                                        return (
                                                ...
                                                <li key={index}>
                                                    ...
                                                    <div className='dis_list'>Q币价： <span>{parseInt(item.goodPrice)/100} Q币</span></div>
